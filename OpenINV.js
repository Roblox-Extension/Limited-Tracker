async function Find_One__Element(Element){
    return new Promise((resolve)=>{
        const interval = setInterval(()=>{
            const T__Element = document.querySelector(Element);
            if(T__Element){
                resolve(T__Element);
                clearInterval(interval)
            }

        },2000)

    })
};
 

async function Find__array(array) {
    return new Promise((resolve)=>{
        const interval = setInterval(()=>{
            const T__Array = document.querySelectorAll(array);
            if(T__Array.length > 2){
                resolve(T__Array);
                console.log(`First element in ${array}: ${T__Array[1]}`);
                clearInterval(interval);
            }

        },2000)

    })

};

async function add_LI () {
    const Inv_Li = document.createElement("li");
    Inv_Li.id = "Inventory_open";
    Inv_Li.classList.add("font-header-2")
    const placeHolder = await Find_One__Element("#nav-inventory");
    if(placeHolder){
    Inv_Li.innerHTML = `
    <svg xmlns="http://www.w3.org/2000/svg" height="23" width="23" viewBox="0 0 512 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.--><path fill="#ffffff" d="M256 0c17.7 0 32 14.3 32 32l0 10.4c93.7 13.9 167.7 88 181.6 181.6l10.4 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-10.4 0c-13.9 93.7-88 167.7-181.6 181.6l0 10.4c0 17.7-14.3 32-32 32s-32-14.3-32-32l0-10.4C130.3 455.7 56.3 381.7 42.4 288L32 288c-17.7 0-32-14.3-32-32s14.3-32 32-32l10.4 0C56.3 130.3 130.3 56.3 224 42.4L224 32c0-17.7 14.3-32 32-32zM107.4 288c12.5 58.3 58.4 104.1 116.6 116.6l0-20.6c0-17.7 14.3-32 32-32s32 14.3 32 32l0 20.6c58.3-12.5 104.1-58.4 116.6-116.6L384 288c-17.7 0-32-14.3-32-32s14.3-32 32-32l20.6 0C392.1 165.7 346.3 119.9 288 107.4l0 20.6c0 17.7-14.3 32-32 32s-32-14.3-32-32l0-20.6C165.7 119.9 119.9 165.7 107.4 224l20.6 0c17.7 0 32 14.3 32 32s-14.3 32-32 32l-20.6 0zM256 224a32 32 0 1 1 0 64 32 32 0 1 1 0-64z"/></svg>
    <span>Trackings</span>
    <style>
    #Inventory_open{
    display: flex;
    flex-direction: row;
    gap: 5px;
    align_items: center;
    cursor: pointer;
    }
    </style>
    `;
    placeHolder.after(Inv_Li);
    console.log("Inventory li Added")
        Inv_Li.addEventListener("click", (e) => {
            const TargetUrl = chrome.runtime.getURL("View/TrackingINV.html")
            window.open(TargetUrl, "_blank");
        })
    }
};

async function main() {
   await add_LI();
}
main();